...
experience () {
 case $1 in
  sight)
   if [ $(($RANDOM%4)) -eq 0 ]; then
    echoes "${sights[$(($RANDOM%${#sights[@]}+$indexing))]}"
   else
    echoes "i can see ${sights[$(($RANDOM%${#sights[@]}+$indexing))]}"
   fi
   ;;
  sound)
   if [ $(($RANDOM%4)) -eq 0 ]; then
    echoes "${sounds[$(($RANDOM%${#sounds[@]}+$indexing))]}"
   else
    echoes "i can hear ${sounds[$(($RANDOM%${#sounds[@]}+$indexing))]}"
   fi
   ;;
  smell)
   if [ $(($RANDOM%4)) -eq 0 ]; then
    echoes "i can smell ${smells[$(($RANDOM%${#smells[@]}+$indexing))]}."
   else
    echoes "${smells[$(($RANDOM%${#smells[@]}+$indexing))]}."
   fi
   ;;
  taste)
   if [ $(($RANDOM%4)) -eq 0 ]; then
    echoes "aftertaste of ${tastes[$(($RANDOM%${#tastes[@]}+$indexing))]}."
   else
    echoes "the aftertaste of ${tastes[$(($RANDOM%${#tastes[@]}+$indexing))]}."
   fi
   ;;
  touch)
   if [ $(($RANDOM%8)) -eq 0 ]; then
    ...
    echoes "${touches[$(($RANDOM%${#touches[@]}+$indexing))]}"
   fi
   ;;
  sensation)
   echoes "${sensations[$(($RANDOM%${#sensations[@]}+$indexing))]}"
   ;;
  *)
   reader=0
   ;;
